...
if (isset($_POST)) {
    $errors = 0;
    if ((!empty($_POST['avis']) && $_POST['avis'] !='' ) || (!empty($_POST['note']) && intval($_POST['note']) > 0 && intval($_POST['note']) < 6)  ) {
        $errors = 0;
        if (strlen($_POST['avis']) > 300) {
            ...
            sql("UPDATE `note` SET `note`=:note,`avis`=:avis WHERE `id_note` = $_POST[id_note]", array(
                'avis' =>  $avis,
                'note' =>  $note
            ));
